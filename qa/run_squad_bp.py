...
#ALL_MODELS = sum(
#  (tuple(XLM_PRETRAINED_CONFIG_ARCHIVE_MAP.keys())),
#  (),
#)
...
class LMQA(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.base_model =  load_adapter(args.modelkg_dir, self.base_model)
    ...
    def forward(self,
        ...
        # print('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%the last hidden is {}'.format(sequence_output.size()))
        # print('================================>{}'.format(sequence_output.size()))
        ...
        # print('===============================================value of logits is {}'.format(logits))
        # print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>shape of logits is {}'.format(logits.size()))
        output = (start_logits, end_logits)
        # print(output) ([384], [384])
...
def evaluate(args, model, tokenizer, prefix="", language='en', lang2id=None):
 ...
 # print('=============================================')
 # print(np.array(examples).shape, np.array(features).shape) #Type: list list   shape:(10570,) (10919,)
 # print(np.array(all_results).shape)  # Type: [<processors.squad.SquadResult object at 0x2b9898c6a310>]   (10919)
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache and not output_examples:
    ...
    if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
      ...
      if evaluate:
        ...
        #print('#####################################################====>type of examples is {}'.format(type(examples)))
        #print('#####################################################====> examples is {}'.format(examples))
      else:
        examples = processor.get_train_examples(args.data_dir, filename=args.train_file, language=language)

    #examples = processor.get_train_examples(args.data_dir, filename=args.train_file, language=language)
    #print(examples, tokenizer)
...
def main():
  ...
  parser.add_argument(
    ...
    default=None,
    type=str,
    help="The style of model inclusing mbert-KG and xlmr-KG",
  )
  parser.add_argument(
    "--modelkg_dir",
    default=None,
    type=str,
    help="The directionary of KG pre-training model",
  )
  parser.add_argument(
    "--model_name_or_path",
    default=None,
  ...
  if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    # Take care of distributed/parallel trainin
    model_to_save = model.module if hasattr(model, "module") else model
  ...
  if args.do_eval and args.local_rank in [-1, 0]:
    ...
    for checkpoint in checkpoints:
      ...
      load_model(model, os.path.join(checkpoint, "pytorch_model.bin"))
      model.to(args.device)
