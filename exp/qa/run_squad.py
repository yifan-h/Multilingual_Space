...
def main():
  ...
  model = load_adapter(args.modelkg_dir, model)
  '''
  model = LMQA(args)
  print(config)
  ...
  if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    # model = model_class.from_pretrained(args.output_dir, force_download=False)
    model = LMQA(args)
  ...
  if args.do_eval and args.local_rank in [-1, 0]:
    ...
    for checkpoint in checkpoints:
      ...
      # model = model_class.from_pretrained(checkpoint, force_download=False)
      load_model(model, os.path.join(args.output_dir, "pytorch_model.bin"))
      #for name, param in model.named_parameters():
      #  if param.requires_grad: print(name)
      model.to(args.device)
